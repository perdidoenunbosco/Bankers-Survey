...
function feedURLs(){
  ...
  URLs = [
    // odd how putting recession at the top makes in last in the array and visa versa
    // this only happens sometimes. this is a aysnc type thing
  ...
  URLs.forEach(URL => {
    console.log(URL);
    initChart(URL, handleQueryResponse);
    console.log("left initchart from feedURLs()"); 

  });
} // END feedURLs

function initChart(URL, callback) {  
  console.log("entering initChart");
    ...
    // this did not work to enforce position in array
    callback(response)
